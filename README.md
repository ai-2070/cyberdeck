...
- **Per-stream credit-window back-pressure.** Stream multiplexing v1 already surfaces `StreamError::WouldBlock` when a local stream queue is full, but there is no round-trip credit accounting between peers — a fast sender can still outrun a slow receiver until the local queue fills. Explicit per-peer credit windows are the next increment.
