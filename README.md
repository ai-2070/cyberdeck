...
Net is a working protocol, not a paper design. 977 tests verify the implementation across every layer of the stack.
...
- **Multi-hop routing alternates.** ProximityGraph-guided rerouting queries `path_to()` for multi-hop alternates per destination and falls back to any direct peer when the graph has no suggestion. Pingwave-driven distance-vector route *installation* (as opposed to reroute-time path lookup) is still a separate plan.
...
- **Automatic rerouting.** When a relay node dies, the `ReroutePolicy` watches the failure detector and automatically updates the routing table to route through an alternate peer. Alternates are picked **per destination** rather than shared across all affected routes — a heterogeneous topology won't blackhole traffic through a peer that reaches some destinations but not others. When the failed peer recovers, the original route is restored. No manual intervention, no data loss across the reroute boundary.
...
- **Round-trip credit-window back-pressure.** v1 back-pressure catches *concurrent callers* racing on the same stream via a local in-flight counter — it does not catch a single serial sender outrunning a slow receiver across the network. v2 will extend the same `StreamError::Backpressure` signal to network-speed by introducing per-peer credit windows (`bytes_in_flight` / `StreamWindow` control message). Daemon code won't change; only the internal condition that triggers Backpressure moves from "local counter full" to "no credit from peer". Design: [`STREAM_BACKPRESSURE_PLAN.md`](net/crates/net/docs/STREAM_BACKPRESSURE_PLAN.md).
- **Pingwave-driven distance-vector route installation.** Today `ReroutePolicy` queries `ProximityGraph::path_to()` on demand at reroute time. A proper DV layer that installs routes from pingwave advertisements, ages them out, and prefers lower-metric paths is a separate plan.
