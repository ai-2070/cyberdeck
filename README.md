...
It's the wrong design for now. Data is abundant. Nodes are abundant. Bandwidth is abundant. The bottleneck isn't delivery - it's processing. A delivery guarantee is only as good as the weakest node in the chain. TCP guarantees delivery to a buffer. It doesn't guarantee the receiver can act on it in time. The guarantee creates false confidence: arrival does not equal usefulness.
...
**Multiplayer gaming.** No dedicated server. Players' devices form the mesh. Game state propagates peer-to-peer with causal ordering. A player drops, the mesh reroutes. Latency is lower than going through a data center because the data never leaves the local mesh.
