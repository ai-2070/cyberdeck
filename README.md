...
- **Multi-route-per-destination routing table.** `RoutingTable::lookup_alternate` is scaffolded but today's table stores one entry per destination, so the only "alternate" is the installed entry if it isn't excluded. When the table grows to hold ranked alternates — and to pick among equal-metric candidates using the latency EWMA populated on each `ProximityGraph` edge — reroute gets deterministic without needing a graph BFS.
