...
Use **scoped capability discovery** with reserved `scope:*` tags. Tag the provider's capability set with `scope:tenant:<id>` (or `scope:region:<name>`, or `scope:subnet-local`); have the consumer call `mesh.find_nodes_scoped(filter, &ScopeFilter::Tenant("<id>"))`. (The scope is taken by reference: `&ScopeFilter<'_>` — `sdk/src/mesh.rs:749-752`. Variants: `Tenant(&str)`, `Region(&str)`, `SameSubnet`, `Any`, `GlobalOnly`.) The consumer gets only the matching pool plus any untagged "Global" providers (permissive default — opt *in* to narrowing, never out by accident).
