...
impl EntityId {
    fn origin_hash(&self) -> u32    // BLAKE2s-MAC keyed "nltp-origin-v1", truncated to 4 bytes
    fn node_id(&self) -> u64        // BLAKE2s-MAC keyed "nltp-node-id-v1", truncated to 8 bytes
    fn verify(&self, message: &[u8], signature: &Signature) -> Result<(), EntityError>
}
```

- `origin_hash()` maps to the `origin_hash` field in every NLTP header (4 bytes)
