...
Formalizes the 16-bit `subprotocol_id` field in every NLTP header. Provides a registry for protocol handlers, version negotiation between peers, and an opaque forwarding guarantee for unknown protocols.

## Subprotocol IDs

Every NLTP packet carries a `subprotocol_id: u16` identifying how the payload should be interpreted. The ID space is partitioned:
