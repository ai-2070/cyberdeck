...
| Test | What it proves |
|------|---------------|
| `test_migration_full_lifecycle_over_wire` | A (orchestrator) migrates a daemon from B (source) to C (target). Full 6-phase lifecycle over actual encrypted UDP — `TakeSnapshot` → `SnapshotReady` → `RestoreComplete` → `BufferedEvents` → `ReplayComplete` → `CutoverNotify` → `CleanupComplete` → `ActivateTarget` → `ActivateAck` chain autonomously. C is provisioned with a `DaemonFactoryRegistry` entry. Asserts daemon ends registered on C, unregistered from B, orchestrator record cleared. Verifies Mikoshi works over the real transport, not just in-process simulations. |
