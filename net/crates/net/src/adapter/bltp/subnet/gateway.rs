...
use crate::adapter::bltp::channel::{ChannelConfigRegistry, Visibility};
            ...
            Visibility::Exported => {
                if let Some(targets) = self.export_table.get(&channel_hash) {
                    if targets.iter().any(|t| {
                        t.is_same_subnet(dest_subnet) || t.is_ancestor_of(dest_subnet)
                    }) {
                        ForwardDecision::Forward
                    } else {
...
mod tests {
    use super::*;
    use crate::adapter::bltp::channel::{ChannelConfig, ChannelId};

    use crate::adapter::bltp::channel::ChannelName;
