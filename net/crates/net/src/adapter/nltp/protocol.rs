//! NLTP wire protocol definitions.
//!
//! This module defines the packet format for the Net L0 Transport Protocol (NLTP).
...
/// NLTP packet header - 64 bytes, cache-line aligned.
///
/// Wire format:
/// ```text
///  0                   1                   2                   3
...
const _: () = assert!(std::mem::size_of::<NltpHeader>() == HEADER_SIZE);
...
mod tests {
    ...
    #[test]
    fn test_header_size() {
        assert_eq!(std::mem::size_of::<NltpHeader>(), HEADER_SIZE);
    }
    ...
    fn test_header_validation() {
        let header = NltpHeader::new(0, 0, 0, [0u8; NONCE_SIZE], 1024, 0, PacketFlags::NONE);
        assert!(header.validate());

        // Invalid magic
        let mut bytes = header.to_bytes();
        bytes[0] = 0xFF;
        let invalid = NltpHeader::from_bytes(&bytes);
    ...
    fn test_validate_rejects_excessive_event_count() {
        let header = NltpHeader::new(0, 0, 0, [0u8; NONCE_SIZE], 100, 10, PacketFlags::NONE);
        assert!(header.validate());
        ...
        let header = NltpHeader::new(
            ...
            [0u8; NONCE_SIZE],
            100,
            NltpHeader::MAX_EVENTS_PER_PACKET + 1,
            PacketFlags::NONE,
        );
        assert!(!header.validate());
    }
