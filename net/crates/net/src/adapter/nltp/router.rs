//! NLTP Router for single-hop and multi-hop packet routing.
//!
//! The router handles:
...
impl NltpRouter {
    ...
    pub fn route_packet(&self, data: Bytes, _from: SocketAddr) -> Result<RouteAction, RouterError> {
        ...
        // Extract stream ID from NLTP header if present
        let stream_id = if data.len() >= ROUTING_HEADER_SIZE + HEADER_SIZE {
            let nltp_header = &data[ROUTING_HEADER_SIZE..ROUTING_HEADER_SIZE + HEADER_SIZE];
            u64::from_le_bytes(nltp_header[32..40].try_into().unwrap_or([0; 8]))
        } else {
            0
        };
...
#[cfg(test)]
mod tests {
    use super::super::protocol::NltpHeader;
    use super::*;
    ...
    async fn test_router_creation() {
        ...
        let router = NltpRouter::new(config).await.unwrap();
    ...
    async fn test_router_extracts_stream_id_at_correct_offset() {
        ...
        // instead of bytes 40..48 where it actually lives in the NLTP header.
        ...
        // Build a NLTP header with a known stream_id
        let nltp = NltpHeader::new(
            0xAAAA,             // session_id
