//! Session and stream state management for NLTP.
//!
...
impl std::fmt::Debug for NltpSession {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NltpSession")
...
pub struct SessionManager {
    ...
    session: parking_lot::RwLock<Option<Arc<NltpSession>>>,
    /// Session timeout
    timeout: Duration,
}
...
impl SessionManager {
    ...
    pub fn set_session(&self, session: NltpSession) {
        let mut guard = self.session.write();
        *guard = Some(Arc::new(session));
    }

    /// Set the current session from an existing Arc
    pub fn set_session_arc(&self, session: Arc<NltpSession>) {
        let mut guard = self.session.write();
        *guard = Some(session);
    }

    /// Get the current session
    pub fn get_session(&self) -> Option<Arc<NltpSession>> {
...
mod tests {
    ...
    fn test_session_creation() {
        ...
        let session = NltpSession::new(keys.clone(), peer_addr, 4, false);
    ...
    fn test_session_streams() {
        ...
        let session = NltpSession::new(keys, peer_addr, 4, false);
    ...
    fn test_session_manager_arc_shares_touch_updates() {
        ...
        let keys = test_keys();
        let peer_addr: SocketAddr = "127.0.0.1:9999".parse().unwrap();
        let session = Arc::new(NltpSession::new(keys, peer_addr, 4, false));
