//! Net L0 Transport Protocol (NLTP) adapter.
//!
//! NLTP is a high-performance UDP-based transport protocol designed for
...
//!
//! ```rust,ignore
//! use net::adapter::nltp::{NltpAdapter, NltpAdapterConfig, StaticKeypair};
//!
//! // Generate keypair for responder
//! let keypair = StaticKeypair::generate();
//!
//! // Create initiator config
//! let config = NltpAdapterConfig::initiator(
...
//!
//! // Create adapter
//! let mut adapter = NltpAdapter::new(config)?;
//! adapter.init().await?;
//! ```
...
pub use transport::{NltpSocket, PacketReceiver, PacketSender, ParsedPacket, SocketBufferConfig};
...
/// NLTP adapter for high-performance UDP transport.
pub struct NltpAdapter {
    /// Configuration
    config: NltpAdapterConfig,
    /// UDP socket
    socket: Option<Arc<Socket>>,
    /// Session (stored separately for init)
    session: Option<Arc<NltpSession>>,
    /// Session manager
...
impl NltpAdapter {
    /// Create a new NLTP adapter.
    pub fn new(config: NltpAdapterConfig) -> Result<Self, AdapterError> {
        config
            .validate()
...
impl Adapter for NltpAdapter {
    async fn init(&mut self) -> Result<(), AdapterError> {
        ...
        let session = Arc::new(NltpSession::new(
            keys,
            actual_peer,
    ...
    async fn shutdown(&self) -> Result<(), AdapterError> {
        ...
        tracing::info!("NLTP adapter shutdown complete");
...
impl std::fmt::Debug for NltpAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NltpAdapter")
...
mod tests {
    ...
    fn test_adapter_creation() {
        ...
        let adapter = NltpAdapter::new(config).unwrap();
        assert_eq!(adapter.name(), "nltp");
    }
    ...
    fn test_invalid_config() {
        ...
        let mut config = NltpAdapterConfig::initiator(
            "127.0.0.1:0".parse().unwrap(),
            "127.0.0.1:9999".parse().unwrap(),
            psk,
            peer_pubkey,
        ...
        let result = NltpAdapter::new(config);
