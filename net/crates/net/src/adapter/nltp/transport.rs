...
use super::protocol::{NltpHeader, HEADER_SIZE, MAX_PACKET_SIZE};
...
/// NLTP socket wrapper with optimized settings.
pub struct NltpSocket {
...
impl NltpSocket {
    /// Create a new NLTP socket bound to the given address with default (production) buffer sizes.
    pub async fn new(bind_addr: SocketAddr) -> io::Result<Self> {
        Self::with_config(bind_addr, SocketBufferConfig::default()).await
    }

    /// Create a new NLTP socket with custom buffer configuration.
...
impl std::fmt::Debug for NltpSocket {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NltpSocket")
...
impl ParsedPacket {
    ...
    pub fn parse(data: Bytes, source: SocketAddr) -> Option<Self> {
        ...
        let header = NltpHeader::from_bytes(&data)?;
        if !header.validate() {
            return None;
        }
...
mod tests {
    ...
    async fn test_socket(addr: SocketAddr) -> io::Result<NltpSocket> {
        NltpSocket::with_config(addr, SocketBufferConfig::for_testing()).await
    }
