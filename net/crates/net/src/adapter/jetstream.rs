...
impl Adapter for JetStreamAdapter {
    ...
    async fn on_batch(&self, batch: Batch) -> Result<(), AdapterError> {
        ...
        // returns a `PublishAckFuture` once enqueued — fast), then await
        // every server ack in parallel. With one ack per event the old
        // serial-await loop paid 1 RTT *per event*; pipelining drops
        // that to ~1 RTT per batch.
        //
        // The message-id buffer (`Nats-Msg-Id` header) is reused across
        // iterations: the `{shard_id}:{seq_start}` prefix is the same
        // for every event in the batch, so we render it once and only
        // rewrite the trailing `:{i}` per event, eliminating the
        // per-event `format!` allocation called out in PERF #2.
        use std::fmt::Write;
        ...
        for (i, data) in serialized.into_iter().enumerate() {
            ...
            // so it can't go straight into `try_join_all`. Wrap each in
            // an async block that handles the await and the error
