...
fn create_link_configs(
    ...
    let resp_cfg =
        NetAdapterConfig::responder(responder.addr, initiator.addr, *psk, responder.keypair.clone())
            .with_handshake(3, Duration::from_secs(3))
            .with_heartbeat_interval(Duration::from_millis(200))
            .with_session_timeout(Duration::from_secs(5))
            .with_socket_buffers(TEST_BUFFER_SIZE, TEST_BUFFER_SIZE);
...
async fn connect_link(
    init_cfg: NetAdapterConfig,
    resp_cfg: NetAdapterConfig,
) -> Link {
    ...
    let (resp, init) = tokio::time::timeout(
        timeout,
        futures::future::join(resp_handle, init_handle),
    )
    .await
...
async fn test_three_node_handshake() {
    ...
    assert!(triangle.link_ab.initiator.is_healthy().await, "A→B initiator unhealthy");
    assert!(triangle.link_ab.responder.is_healthy().await, "A→B responder unhealthy");
    assert!(triangle.link_ac.initiator.is_healthy().await, "A→C initiator unhealthy");
    assert!(triangle.link_ac.responder.is_healthy().await, "A→C responder unhealthy");
    assert!(triangle.link_bc.initiator.is_healthy().await, "B→C initiator unhealthy");
    assert!(triangle.link_bc.responder.is_healthy().await, "B→C responder unhealthy");
...
async fn test_bidirectional_simultaneous() {
    ...
    triangle.link_ab.initiator.on_batch(batch_a_to_b).await.expect("A→B send failed");
    tokio::time::sleep(Duration::from_millis(100)).await;
    triangle.link_ab.responder.on_batch(batch_b_to_a).await.expect("B→A send failed");
...
async fn test_full_ring_traffic() {
    ...
    triangle.link_ab.initiator.on_batch(batch_ab).await.expect("A→B failed");
    triangle.link_bc.initiator.on_batch(batch_bc).await.expect("B→C failed");
    // C→A: C is responder on link_ac
    triangle.link_ac.responder.on_batch(batch_ca).await.expect("C→A failed");
    ...
    assert!(b_got.events.len() > 0, "B should receive from A, got {}", b_got.events.len());
    assert!(c_got.events.len() > 0, "C should receive from B, got {}", c_got.events.len());
    assert!(a_got.events.len() > 0, "A should receive from C, got {}", a_got.events.len());
...
use net::adapter::net::{
    NetRouter, RouteAction, RouterConfig, RouterError,
    RoutingHeader, ROUTING_HEADER_SIZE,
};
...
async fn test_router_forwarding_through_middle_node() {
    ...
    let (n, from) = tokio::time::timeout(
        ...
        router_b.recv_from(&mut recv_buf),
    )
    .await
...
async fn test_router_ttl_expiry() {
    ...
    let stats = router_b.stats();
    assert_eq!(stats.packets_dropped, 1, "TTL-expired packet should be counted as dropped");
    assert_eq!(stats.packets_forwarded, 0, "TTL-expired packet should not be forwarded");
}
...
async fn test_router_hop_count_incremented() {
    ...
    // Route it through B
    let action = router_b.route_packet(packet, "127.0.0.1:0".parse().unwrap()).unwrap();
    ...
    assert_eq!(fwd_header.hop_count, 1, "hop_count should be 1 after one forward");
...
async fn test_eventbus_over_net_full_stack() {
    ...
    let sender_net = NetAdapterConfig::initiator(
        sender_addr,
        receiver_addr,
        psk,
    ...
    let receiver_handle = tokio::spawn(async move {
        ...
        let shard0 = adapter.poll_shard(0, None, 1000).await.expect("poll 0 failed");
        let shard1 = adapter.poll_shard(1, None, 1000).await.expect("poll 1 failed");
    ...
    let sender_handle = tokio::spawn(async move {
        ...
        for i in 0..50 {
            let event = net::event::Event::new(serde_json::json!({"index": i, "source": "eventbus"}));
...
async fn test_backpressure_ring_buffer_survives_flood() {
    ...
    assert!(
        ingested > 0,
        "at least some events should be ingested"
    );
...
async fn test_eventbus_bidirectional_over_net() {
    ...
    let b_handle = tokio::spawn(async move {
        ...
        let events: Vec<InternalEvent> = (0..20)
            ...
            .collect();
        adapter
            .on_batch(Batch { shard_id: 0, events, sequence_start: 0 })
            .await
            .expect("B send failed");

        // Wait for A's events
        tokio::time::sleep(Duration::from_millis(1500)).await;
        let received = adapter.poll_shard(0, None, 1000).await.expect("B poll failed");
    ...
    let a_handle = tokio::spawn(async move {
        ab.wait().await;
        let bus = net::EventBus::new(config_a).await.expect("A EventBus failed");
        tokio::time::sleep(Duration::from_millis(500)).await;

        // A ingests events (→ Net → B)
        for i in 0..20 {
            bus.ingest(net::event::Event::new(serde_json::json!({"from": "A", "i": i})))
                .unwrap();
        }
        ...
        tokio::time::sleep(Duration::from_millis(1500)).await;
        let response = bus.poll(net::ConsumeRequest::new(1000)).await.expect("A poll failed");
    ...
    let timeout = Duration::from_secs(15);
    let (b_result, a_result) = tokio::time::timeout(
        timeout,
        futures::future::join(b_handle, a_handle),
    )
    .await
    ...
    loop {
        ...
        .await
        {
            Ok(Ok((n, _))) => {
                ...
                assert!(hdr.is_some(), "forwarded packet should have valid routing header");
                let hdr = hdr.unwrap();
...
async fn test_router_multi_hop_two_routers() {
    ...
    match action {
        RouteAction::Local(payload) => {
            assert_eq!(&payload[..], b"multi-hop-test", "payload should survive 2 hops");
        }
...
use net::adapter::net::{
    SubnetGateway, SubnetId, ForwardDecision, DropReason,
    ChannelConfig, ChannelConfigRegistry, ChannelId, Visibility,
};
...
async fn test_subnet_gateway_forwards_global_traffic() {
    ...
    assert_eq!(decision, ForwardDecision::Forward, "Global traffic must cross subnet boundary");
}
...
async fn test_subnet_gateway_exported_selective() {
    ...
    assert_eq!(to_c, ForwardDecision::Forward, "exported to C should forward");

    let to_d = gateway.should_forward(subnet_ab, subnet_d, export_hash, 8, 0);
    assert_eq!(to_d, ForwardDecision::Drop(DropReason::NotExported), "not exported to D should drop");
}
...
async fn test_subnet_gateway_parent_visible() {
    ...
    assert_eq!(to_parent, ForwardDecision::Forward, "child to parent should forward");

    let to_sibling = gateway.should_forward(child, sibling, hash, 8, 0);
    assert_eq!(to_sibling, ForwardDecision::Drop(DropReason::NotAncestor), "child to sibling should drop");
}
...
use net::adapter::net::{CorrelatedFailureDetector, CorrelatedFailureConfig, CorrelationVerdict};
...
async fn test_correlated_failure_independent() {
    ...
    assert_eq!(budget, usize::MAX, "independent failures get unlimited budget");
}
...
async fn test_correlated_failure_mass() {
    ...
    let budget = detector.recovery_budget();
    assert_eq!(budget, 3, "mass failure throttles to max_concurrent_migrations");
...
use net::adapter::net::{FailureDetector, FailureDetectorConfig, NodeStatus};
...
async fn test_pingwave_forwarding_chain() {
    ...
    let mut pw = Pingwave::new(node_a, 1, 3);
