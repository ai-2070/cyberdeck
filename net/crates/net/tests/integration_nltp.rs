//! End-to-end integration tests for NLTP adapter.
//!
//! These tests verify the full NLTP transport flow including:
...
//!
//! Run tests:
//!   cargo test --features nltp --test integration_nltp
...
fn create_config_pair(
    initiator_port: u16,
    responder_port: u16,
) -> (NltpAdapterConfig, NltpAdapterConfig) {
    let psk = [0x42u8; 32];
    ...
    let initiator_config = NltpAdapterConfig::initiator(
        initiator_addr,
        responder_addr,
        psk,
    ...
    let responder_config =
        NltpAdapterConfig::responder(responder_addr, initiator_addr, psk, responder_keypair)
            .with_handshake(3, Duration::from_secs(2))
            .with_heartbeat_interval(Duration::from_millis(500))
            .with_session_timeout(Duration::from_secs(5))
            .with_socket_buffers(TEST_BUFFER_SIZE, TEST_BUFFER_SIZE);
...
async fn test_nltp_handshake() {
    ...
    let responder_handle = tokio::spawn(async move {
        let mut adapter = net::adapter::nltp::NltpAdapter::new(responder_config).unwrap();
    ...
    let initiator_handle = tokio::spawn(async move {
        let mut adapter = net::adapter::nltp::NltpAdapter::new(initiator_config).unwrap();
...
#[tokio::test]
async fn test_nltp_send_receive_fire_and_forget() {
    let (port1, port2) = find_available_ports().await;
    let (initiator_config, responder_config) = create_config_pair(port1, port2);
...
#[tokio::test]
async fn test_nltp_reliable_mode() {
    let (port1, port2) = find_available_ports().await;
...
#[tokio::test]
async fn test_nltp_multiple_streams() {
    let (port1, port2) = find_available_ports().await;
    let (initiator_config, responder_config) = create_config_pair(port1, port2);
...
#[tokio::test]
async fn test_nltp_health_check() {
    let (port1, port2) = find_available_ports().await;
    let (initiator_config, responder_config) = create_config_pair(port1, port2);
...
#[tokio::test]
async fn test_nltp_large_batch() {
    let (port1, port2) = find_available_ports().await;
    let (initiator_config, responder_config) = create_config_pair(port1, port2);
...
#[tokio::test]
async fn test_nltp_adapter_name() {
    let psk = [0x42u8; 32];
    ...
    let adapter = net::adapter::nltp::NltpAdapter::new(config).unwrap();
    assert_eq!(adapter.name(), "nltp");
}

#[tokio::test]
async fn test_nltp_flush() {
...
mod unit {
    ...
    fn test_packet_header_roundtrip() {
        ...
        let header = NltpHeader::new(0x1234, 0x5678, 42, nonce, 100, 5, PacketFlags::RELIABLE);

        let bytes = header.to_bytes();
        assert_eq!(bytes.len(), HEADER_SIZE);

        let parsed = NltpHeader::from_bytes(&bytes).unwrap();
