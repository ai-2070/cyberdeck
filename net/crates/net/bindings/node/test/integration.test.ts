/**
 * Integration tests for Net Node.js bindings with Net.
 *
 * Run tests:
 *   npm test
 *
...
try {
  if (typeof generateNetKeypair === "function") {
    generateNetKeypair();
    netAvailable = true;
  }
} catch {
...
describe.skipIf(!RUN_INTEGRATION_TESTS || !netAvailable)(
  "Net Integration Tests",
  () => {
    it("should generate valid keypairs", () => {
      const keypair = generateNetKeypair();
      ...
      const keypair2 = generateNetKeypair();
    ...
    it("should exchange events between initiator and responder", async () => {
      ...
      const responderKeypair = generateNetKeypair();
    ...
    it("should support batch ingestion over Net", async () => {
      const responderKeypair = generateNetKeypair();
      const psk = crypto.randomBytes(32).toString("hex");
