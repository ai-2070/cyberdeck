...
```python
from net import Net, generate_nltp_keypair
import os
...
responder = Net(
    num_shards=2,
    nltp_bind_addr='127.0.0.1:9001',
    nltp_peer_addr='127.0.0.1:9000',
    nltp_psk=psk,
    nltp_role='responder',
    nltp_secret_key=keypair.secret_key,
    nltp_public_key=keypair.public_key,
    nltp_reliability='light',  # 'none', 'light', or 'full'
)

# Initiator side (knows responder's public key)
initiator = Net(
    num_shards=2,
    nltp_bind_addr='127.0.0.1:9000',
    nltp_peer_addr='127.0.0.1:9001',
    nltp_psk=psk,
    nltp_role='initiator',
    nltp_peer_public_key=keypair.public_key,
)
