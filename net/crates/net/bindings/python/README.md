...
(`producer_nonce_path` on `EventBusConfig`, BUG #56), the id is
stable across both within-process retries AND cross-process
restart — the `MULTI/EXEC`-timeout race (BUG #57) becomes
...
cursor = "0"
while True:
    entries = r.xrange("net:shard:0", min=cursor, max="+", count=100)
...
Background: [`docs/BUG_AUDIT_2026_04_30_CORE.md`](../../docs/BUG_AUDIT_2026_04_30_CORE.md)
entries #56 and #57.
