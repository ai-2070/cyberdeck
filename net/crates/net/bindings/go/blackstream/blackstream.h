...
typedef void* blackstream_handle_t;
 ...
 * }
 */
blackstream_handle_t blackstream_init(const char* config_json);
 ...
 */
int blackstream_ingest(blackstream_handle_t handle, const char* event_json, size_t len);
 ...
 * @return 0 on success, negative error code on failure.
 */
int blackstream_ingest_raw(blackstream_handle_t handle, const char* json, size_t len);
 ...
 */
int blackstream_ingest_raw_batch(
    blackstream_handle_t handle,
    const char** jsons,
    const size_t* lens,
    size_t count
);
 ...
 */
int blackstream_ingest_batch(blackstream_handle_t handle, const char* events_json);
 ...
 * @return Bytes written on success, negative error code on failure.
 */
int blackstream_stats(blackstream_handle_t handle, char* out_buffer, size_t buffer_len);
 ...
 * @return 0 on success, negative error code on failure.
 */
int blackstream_flush(blackstream_handle_t handle);
 ...
 */
uint16_t blackstream_num_shards(blackstream_handle_t handle);
 ...
 * @return 0 on success, negative error code on failure.
 */
int blackstream_shutdown(blackstream_handle_t handle);
 ...
 */
const char* blackstream_version(void);

/*
 * Generate a new BLTP keypair (requires BLTP feature).
 * 
 * @return JSON string with hex-encoded public_key and secret_key,
 *         or NULL if BLTP is not enabled. Caller must free with blackstream_free_string.
 */
char* bltp_generate_keypair(void);
 ...
 */
void blackstream_free_string(char* s);
