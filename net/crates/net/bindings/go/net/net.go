...
// ingestion and consumption with support for the NLTP encrypted UDP backend.
//
// # Quick Start
//
//	bus, err := net.New(nil)
...
type Config struct {
	...
	// Nltp configuration for NLTP encrypted UDP backend.
	Nltp *NltpConfig `json:"nltp,omitempty"`
}

// NltpConfig represents NLTP encrypted UDP adapter configuration.
type NltpConfig struct {
...
// NltpKeypair holds a generated keypair for NLTP.
type NltpKeypair struct {
...
// GenerateNltpKeypair generates a new X25519 keypair for NLTP.
//
...
// with the initiator.
func GenerateNltpKeypair() (*NltpKeypair, error) {
	result := C.nltp_generate_keypair()
	if result == nil {
		return nil, errors.New("failed to generate keypair (NLTP feature may not be enabled)")
	}
	defer C.net_free_string(result)

	jsonStr := C.GoString(result)
	var keypair NltpKeypair
