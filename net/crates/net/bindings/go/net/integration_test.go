// Integration tests for Net Go bindings with NLTP.
//
// Run tests:
//
...
// nltpAvailable checks if NLTP feature is available
func nltpAvailable() bool {
	_, err := GenerateNltpKeypair()
	return err == nil
}

func skipIfNltpNotEnabled(t *testing.T) {
	skipIfNotEnabled(t)
	if !nltpAvailable() {
		t.Skip("NLTP feature not available (build with NLTP feature enabled)")
	}
}
...
func TestNltpGenerateKeypair(t *testing.T) {
	skipIfNltpNotEnabled(t)

	keypair, err := GenerateNltpKeypair()
	if err != nil {
	...
	keypair2, err := GenerateNltpKeypair()
	if err != nil {
...
func TestNltpExchangeEvents(t *testing.T) {
	skipIfNltpNotEnabled(t)

	// Generate keypair for responder
	responderKeypair, err := GenerateNltpKeypair()
	if err != nil {
		t.Fatalf("Failed to generate keypair: %v", err)
	}
...
func TestNltpBatchIngestion(t *testing.T) {
	skipIfNltpNotEnabled(t)
...
func TestNltpFullReliabilityMode(t *testing.T) {
	skipIfNltpNotEnabled(t)
...
func TestNltpStats(t *testing.T) {
	skipIfNltpNotEnabled(t)
