//! Benchmarks for BLTP adapter throughput.
//!
//! Run with: cargo bench --features bltp --bench bltp
//!
...
//! - Legacy vs Fast cipher comparison
//! - Legacy vs Thread-local pool comparison
...
fn bench_header(c: &mut Criterion) {
    let mut group = c.benchmark_group("bltp_header");
    group.throughput(Throughput::Elements(1));

    let nonce = [0x42u8; NONCE_SIZE];
    let header = BltpHeader::new(0x1234, 0x5678, 42, nonce, 1000, 10, PacketFlags::RELIABLE);
    ...
    group.bench_function("deserialize", |b| {
        b.iter(|| BltpHeader::from_bytes(&header_bytes));
    });

    group.bench_function("roundtrip", |b| {
        b.iter(|| {
            let bytes = header.to_bytes();
            BltpHeader::from_bytes(&bytes)
        });
    });
...
fn bench_event_frame(c: &mut Criterion) {
    let mut group = c.benchmark_group("bltp_event_frame");
...
fn bench_packet_pool(c: &mut Criterion) {
    let mut group = c.benchmark_group("bltp_packet_pool");
    group.throughput(Throughput::Elements(1));
...
fn bench_keypair(c: &mut Criterion) {
    let mut group = c.benchmark_group("bltp_keypair");
    group.throughput(Throughput::Elements(1));
...
fn bench_aad(c: &mut Criterion) {
    let mut group = c.benchmark_group("bltp_aad");
    group.throughput(Throughput::Elements(1));
...
fn bench_pool_comparison(c: &mut Criterion) {
    ...
    group.bench_function("legacy_pool_10x", |b| {
        b.iter(|| {
            for _ in 0..10 {
                let builder = legacy_pool.get();
                drop(builder);
            ...
            |b, &thread_count| {
                b.iter(|| {
                    let handles: Vec<_> = (0..thread_count)
                        .map(|_| {
                            let pool = Arc::clone(&legacy_pool);
                            thread::spawn(move || {
                                let event_data = Bytes::from(vec![0x42u8; 64]);
                                let events = vec![event_data];
...
fn bench_multithread_mixed_frames(c: &mut Criterion) {
    ...
    for thread_count in [8, 16, 24, 32].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("legacy_mixed", thread_count),
            thread_count,
            |b, &thread_count| {
                b.iter(|| {
                    let handles: Vec<_> = (0..thread_count)
...
fn bench_failure_recovery_cycle(c: &mut Criterion) {
    ...
    group.bench_function("full_cycle", |b| {
        ...
        b.iter(|| {
            ...
            if detector.status(id) == blackstream::adapter::bltp::NodeStatus::Failed {
