//! Benchmarks for NLTP adapter throughput.
//!
//! Run with: cargo bench --features nltp --bench nltp
//!
...
fn bench_header(c: &mut Criterion) {
    let mut group = c.benchmark_group("nltp_header");
    group.throughput(Throughput::Elements(1));

    let nonce = [0x42u8; NONCE_SIZE];
    let header = NltpHeader::new(0x1234, 0x5678, 42, nonce, 1000, 10, PacketFlags::RELIABLE);
    ...
    group.bench_function("deserialize", |b| {
        b.iter(|| NltpHeader::from_bytes(&header_bytes));
    });

    group.bench_function("roundtrip", |b| {
        b.iter(|| {
            let bytes = header.to_bytes();
            NltpHeader::from_bytes(&bytes)
        });
    });
...
fn bench_event_frame(c: &mut Criterion) {
    let mut group = c.benchmark_group("nltp_event_frame");
...
fn bench_packet_pool(c: &mut Criterion) {
    let mut group = c.benchmark_group("nltp_packet_pool");
    group.throughput(Throughput::Elements(1));
...
fn bench_packet_build(c: &mut Criterion) {
    let mut group = c.benchmark_group("nltp_packet_build");
...
fn bench_encryption(c: &mut Criterion) {
    let mut group = c.benchmark_group("nltp_encryption");
...
fn bench_keypair(c: &mut Criterion) {
    let mut group = c.benchmark_group("nltp_keypair");
    group.throughput(Throughput::Elements(1));
...
fn bench_aad(c: &mut Criterion) {
    let mut group = c.benchmark_group("nltp_aad");
    group.throughput(Throughput::Elements(1));
...
fn bench_failure_recovery_cycle(c: &mut Criterion) {
    ...
    group.bench_function("full_cycle", |b| {
        ...
        b.iter(|| {
            ...
            if detector.status(id) == net::adapter::nltp::NodeStatus::Failed {
