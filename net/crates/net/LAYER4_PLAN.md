...
Events get a 24-byte `CausalLink` prefix inside EventFrame. The Net header stays
...
pub struct CausalLink {
    pub origin_hash: u32,       // entity identity (matches Net header)
...
Tamper resistance comes from Net's AEAD, not per-event signatures.
...
## New Module: `src/adapter/net/state/`
...
- Transfer via Net fragmentation (`subprotocol_id = 0x0401`)
...
- **xxh3 for parent_hash, not blake2**: blake2 ~2GB/s vs xxh3 ~50GB/s. At 40M events/sec
  blake2 would consume 2 CPU cores. Chain integrity comes from Net AEAD, not per-event crypto.
